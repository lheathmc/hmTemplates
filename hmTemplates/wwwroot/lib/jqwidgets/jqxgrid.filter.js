...
(function ($) {
    ...
    $.extend($.jqx._jqxGrid.prototype, {
        ...
        _addfilterwidget: function (columnrecord, tablecolumn, width) {
            ...
            var addtextfilter = function (me, tablecolumn) {
                ...
                textbox.mouseenter(function () {
                    textbox.addClass(me.toThemeProperty(columnrecord.datafield + "-filter-hover"));
                });
                textbox.mouseleave(function () {
                    textbox.removeClass(me.toThemeProperty(columnrecord.datafield + "-filter-hover"));
                });
                /*** OVER: SRG CODE ***/
                ...
                textbox.keydown(function (event) {
                    /*** SRG CODE: Commented by SRG [Disabled filtering on enter key] - Start ***/
                    //if (event.keyCode == '13')
                    //    me._applyfilterfromfilterrow();
                    /*** SRG CODE: Commented by SRG [Disabled filtering on enter key] - End ***/
                    ...
                    textbox[0]._writeTimer = setTimeout(function () {
                        if (!me._loading) {
                            ...
                            if (me["_oldWriteText" + textbox[0].id] != textbox.val()) {
                                // SRG CODE - START
                                if (columnrecord.limitFilter === true) {
                                    if (textbox.val().trim().length >= 3) {
                                        me._statusFilter = true;
                                        me._applyfilterfromfilterrow();

                                    } else if (textbox.val().trim().length == 0 && me._statusFilter === true) {
                                        me._statusFilter = false;
                                        me._applyfilterfromfilterrow();
                                    }
                                } else {
                                    me._applyfilterfromfilterrow();
                            ...
                            /* Logic has been changed to apply the filter, if minimum 3 chars are entered into the textbox */
                        }
        ...
        addfilter: function (datafield, filter, apply) {
            ...
            // SRG CODE - START
            if (columnbydatafield.limitFilter === true)
                this._statusFilter = true;
            // SRG CODE - END
